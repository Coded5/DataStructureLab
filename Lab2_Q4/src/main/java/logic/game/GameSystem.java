...
public class GameSystem {
    ...
    public BaseUnit promote(BaseUnit baseUnit, int choice) {
        int startColumn = baseUnit.getColumn();
        int startRow = baseUnit.getRow();
        boolean isWhite = baseUnit.isWhite();
        String name = baseUnit.getName();
        switch (choice) {
            case 0:
                return new MeleeUnit(startColumn, startRow, isWhite, name);
            case 1:
                return new RangeUnit(startColumn, startRow, isWhite, name);
            case 2:
                return new FlyingUnit(startColumn, startRow, isWhite, name);
            default:
                return null;
        }
    }
