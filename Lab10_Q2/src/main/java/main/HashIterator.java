...
public class HashIterator implements Iterator {
    ...
    public OpenAddressing h; // the associated hash table
    public int currentPos; // position in the table's array that is currently marked.
    ...
    public int previous() throws Exception {
        ...
        int currentValue = h.array[currentPos];
        while (h.array[--currentPos] <= 0)
            ;
        return currentValue;
    }
