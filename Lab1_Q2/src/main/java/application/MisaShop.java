...
public class MisaShop {
    ...
    private static void handleAddItem() {
        ...
        if (isSuccess) {
            ...
            System.out.println(
                    item.getName() + " (" + item.getPricePerPiece() + ") has been added to item list successfully!");
        } else {
    ...
    private static void showAllOrder() {
        for (Order order : orderList) {
            System.out.println(
                    "Order No." + order.getOrderNumber() + " : total price " + order.calculateOrderTotalPrice());
            ArrayList<OrderItem> itemListInOrder = order.getOrderItemList();
            for (OrderItem orderItem : itemListInOrder) {
                System.out.println(" -- " + orderItem.getItem().getName() + " " + orderItem.getItemAmount()
                        + " piece(s) * " + orderItem.getItem().getPricePerPiece() + " per piece.");
            }
        }
    }
