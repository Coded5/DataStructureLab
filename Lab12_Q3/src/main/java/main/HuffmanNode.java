...
public class HuffmanNode implements Comparable {
    ...
    public static char[] codes = new char[40];
    ...
    public static HuffmanNode buildHuffmanTree(Heap h) throws Exception {
        while (h.size > 1) {
            HuffmanNode node1 = (HuffmanNode) h.pop();
            HuffmanNode node2 = (HuffmanNode) h.pop();

            HuffmanNode sumNode = new HuffmanNode(node1.frequency + node2.frequency, ' ', node1, node2);
            h.add(sumNode);
        }

        HuffmanNode root = (HuffmanNode) h.top();
        return root;
    }
