...
public class BST {
    ...
    public void addLeftSubTreeToMin(BST subtree) {
        ...
        BSTNode minNode = ((TreeIterator) findMin()).currentNode;

        int subtreeMax = ((TreeIterator) subtree.findMax()).currentNode.data;

        if (subtree.root.data >= minNode.data || subtreeMax >= minNode.data)
            return;

        size += subtree.size;
        minNode.left = subtree.root;
        subtree.root.parent = minNode;
    }
