...
public class BSTRecursive {
    ...
    public int[] nonAVLNodes(BSTNode n) {
        ...
        findNonAVL(result, 0, n);

        Arrays.sort(result);
        return result;
    }

    private int findNonAVL(int[] result, int count, BSTNode node) {
        if (node == null)
            return count;
        ...
        int countLeft = findNonAVL(result, count, node.left);
        int countRight = findNonAVL(result, countLeft, node.right);
