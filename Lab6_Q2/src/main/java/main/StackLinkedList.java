...
public class StackLinkedList implements MyStack {
    ...
    public void removeRange(int i, int j) throws Exception {
        DListIterator itrStart = new DListIterator(theList.header);
        DListIterator itrEnd = new DListIterator(theList.header);
        ...
        DListNode start = itrStart.currentNode;
        DListNode end = itrEnd.currentNode;

        start.previousNode.nextNode = end.nextNode;
        end.nextNode.previousNode = start.previousNode;

        int removeSize = j - i + 1;
        this.theList.size -= removeSize;
    }
